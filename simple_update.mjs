...
if (!content.includes('interface AlarmMappingItem')) {
    content = content.replace("interface DeviceSettingsProps", mappingInterface + "\n\ninterface DeviceSettingsProps");
}

const mappingState = `  // Alarm Mapping States
...
if (!content.includes('alarmMappings')) {
    content = content.replace("  const [isTesting, setIsTesting] = useState(false);", "  const [isTesting, setIsTesting] = useState(false);\n" + mappingState);
}
...
if (!content.includes('handleImportClick')) {
    content = content.replace("  const handleTestConnection = async () => {", mappingHandlers + "\n  const handleTestConnection = async () => {");
}
