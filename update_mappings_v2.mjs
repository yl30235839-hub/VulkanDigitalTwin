...
// 1. Add State (more flexible match)
if (!content.includes('alarmMappings')) {
  // Try to find the tableData state with possible leading spaces
  const tableDataPattern = /^\s*const \[tableData, setTableData\] = useState<any\[\]>\(\[\]\);/m;
  const stateCode = `  const [tableData, setTableData] = useState<any[]>([]);
  ...
  if (tableDataPattern.test(content)) {
    content = content.replace(tableDataPattern, stateCode);
    console.log('State added');
  } else {
    console.log('tableData state not found');
  }
}

// 2. Add Handlers (after the useEffect that depends on [device])
if (!content.includes('handleImportClick')) {
  const useEffectPattern = /setFormData\(\{[\s\S]*?\}\);\s*\}\s*\}, \[device\]\);/;
  const handlerCode = `
...
`;
  ...
  if (useEffectPattern.test(content)) {
    content = content.replace(useEffectPattern, (match) => match + handlerCode);
