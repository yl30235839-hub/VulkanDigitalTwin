...
// Force add if not present as a definition
if (!content.includes('const [alarmMappings')) {
    content = content.replace(/const\s+\[isTesting,\s+setIsTesting\]\s+=\s+useState\(false\);/, (m) => m + mappingState);
}

if (!content.includes('const handleImportClick')) {
    content = content.replace(/const\s+handleTestConnection\s+=\s+async\s+\(\)\s+=>\s+\{/, (m) => mappingHandlers + "\n\n" + m);
}

fs.writeFileSync(path, content);
console.log('Force update finish');
