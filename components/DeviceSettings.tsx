...
const DeviceSettings: React.FC<DeviceSettingsProps> = ({
  ...
  const [connectionResult, setConnectionResult] =
    useState<ConnectionResult>("IDLE");
  ...
  const paginatedAlarms = alarmMappings.slice(
    (alarmCurrentPage - 1) * alarmPageSize,
    alarmCurrentPage * alarmPageSize,
  );
  ...
  const [draggedProcessIndex, setDraggedProcessIndex] = useState<number | null>(
    null,
  );
  const [tempProcessMappings, setTempProcessMappings] = useState<
    ProcessMappingItem[]
  >([]);
  ...
  const startProcessMappingEdit = () => {
    ...
    const newTemp = Array.from({ length: totalLength }).map((_, i) => {
      ...
      let autoBit = "-";
      if (
        formData.processReadMethod === "按字讀取" &&
        formData.processParamAddress
      ) {
  ...
  const handleRefreshProcessMappings = async () => {
    ...
    try {
      const response = await api.post(
        `${backendDomain}/api/Equipment/ProcessMapDataRefresh`,
        {
  ...
  const saveProcessMappingEdit = () => {
    const newProcessMappings = tempProcessMappings.map((item, idx) => {
      ...
      let autoBit =
        processMappings[idx]?.parameterBit || item.parameterBit || "-";
  ...
  const handleAddProcessMapping = () => {
    if (selectedProcessType === "AGV訂單管理") {
      const existingCount = processMappings.filter(
        (m) =>
          m.parameterType === "AGV訂單管理" &&
          m.name?.startsWith("AGVOrderSignal"),
      ).length;
      const nextIndex = existingCount + 1;

      const newMapping1: ProcessMappingItem = {
        id: Date.now().toString(),
        name: `AGVOrderSignal${nextIndex}`,
        description: "AGV訂單維護點位",
        function: "Read",
        channelNumber: "1",
        address: "DXXX",
        parameterBit: "0",
        parameterType: "AGV訂單管理",
      };

      const newMapping2: ProcessMappingItem = {
        id: (Date.now() + 1).toString(),
        name: `ReAGVOrderSignal${nextIndex}`,
        description: "AGV訂單回復",
        function: "Write",
        channelNumber: "1",
        address: "DXXX",
    ...
    } else if (selectedProcessType === "工藝結果") {
      const existingCount = processMappings.filter(
        (m) =>
          m.parameterType === "工藝結果" &&
          m.name?.startsWith("PointResultSignal"),
      ).length;
      const nextIndex = existingCount + 1;
      ...
      newProcessDataType === "數值"
    ) {
      const existingCount = processMappings.filter(
        (m) =>
          m.parameterType === "工藝信息" && m.name?.startsWith("NumSignal"),
      ).length;
      const nextIndex = existingCount + 1;
      ...
      const existingCount = processMappings.filter(
        (m) =>
          m.parameterType === "工藝信息" && m.name?.startsWith("StringSignal"),
      ).length;
      const nextIndex = existingCount + 1;
      ...
      const stringValueMappings: ProcessMappingItem[] = Array.from({
        length: newProcessDataLength,
      }).map((_, idx) => ({
      ...
      const existingCount = processMappings.filter(
        (m) =>
          m.parameterType === "工藝排產" && m.name?.startsWith("PointExecute"),
      ).length;
      const nextIndex = existingCount + 1;
      ...
      const newMapping: ProcessMappingItem = {
        ...
        processDataType:
          selectedProcessType === "工藝信息" ? newProcessDataType : undefined,
        processDataLength:
          selectedProcessType === "工藝信息" && newProcessDataType === "字符串"
            ? newProcessDataLength
            : undefined,
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && device) {
      ...
      try {
        ...
        const response = await api.post(
          `${backendDomain}/api/Equipment/AlarmMapDataImport`,
          importFormData,
          {
            headers: {
        ...
        if (response.data && response.data.code === 200 && response.data.data) {
          ...
          const newMappings: AlarmMappingItem[] = alarmData.map(
      ...
      reader.onload = (event) => {
        try {
          ...
          const rows = fileContent
            .split("\n")
            .filter((row) => row.trim() !== "");
          const newMappings: ProcessMappingItem[] = rows
            .slice(1)
            .map((row, index) => {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const response = await fetch(
        `${backendDomain}/api/Equipment/CommLinkTest`,
        {
          method: "POST",
          headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      if (
        ...
        device.type === EquipmentType.TestingEquipment ||
        device.type === EquipmentType.WaterVaporEquipment
      ) {
        ...
        const craftsPayload = processMappings.map((item, index) => {
          let typeStr = item.parameterType || "";
          switch (typeStr) {
            case "設備信息":
              typeStr = "EquipmentInfo";
              break;
            case "工藝結果":
              typeStr = "CraftResult";
              break;
            case "工藝排產":
              typeStr = "CraftScheduling";
              break;
            case "工藝信息":
              typeStr = "CraftInfo";
              break;
          ...
          let autoAddress = item.address || "";
          let autoBit = item.parameterBit || "";
          ...
          if (
            ...
            const match =
              formData.processParamAddress.match(/^([A-Za-z]+)(\d+)$/);
            if (match) {
              const prefix = match[1];
              const startNum = parseInt(match[2], 10);
        ...
        const hopperInfos = storageLocations.map((loc) => {
          let reqType = "";
          if (loc.requestType === "上料") {
            reqType = "Up";
          } else if (loc.requestType === "下料") {
            reqType = "Down";
          }
          ...
          if (loc.priority === "正常") {
            prio = "0";
          } else if (loc.priority === "緊急") {
            prio = "1";
          } else if (loc.priority === "非常緊急") {
            prio = "2";
          }

          return {
            signalName: loc.productInfo || "",
            equipmentSysName: device.id || "",
            lineSysName: device.lineId || "",
            requestType: reqType,
            priority: prio,
            num: loc.quantity || 0,
            orderAddress: loc.orderAddress || "",
            reOrderAddress: loc.replyAddress || "",
          };
        });
        ...
        const response = await api.post(
          `${backendDomain}/api/Equipment/CEMaintenance`,
          {
            lineSystemName: device.lineId,
            equipmentSystemName: device.id,
            equipmentName: formData.name,
  ...
  const handleConnectDB = () => {
    ...
    setTimeout(() => {
      ...
      setDbTables([
        ...
        "Machine_Errors_History",
        "Sensor_Readings_Realtime",
        "Employee_Shifts_Config",
        "System_Audit_Log",
      ]);
  ...
  const addNewRow = () => {
    const newId =
      editingRows.length > 0
        ? Math.max(...editingRows.map((r) => r.id)) + 1
        : 1000;
    const newRow = {
      id: newId,
      timestamp: new Date().toLocaleString(),
  ...
  const updateRowValue = (id: number, field: string, value: string) => {
    setEditingRows(
      editingRows.map((r) => (r.id === id ? { ...r, [field]: value } : r)),
    );
  };
  ...
  const handleRefreshAlarmMappings = async () => {
    ...
    try {
      const response = await api.post(
        `${backendDomain}/api/Equipment/AlarmMapDataRefresh`,
        {
          equipmentSystemName: device.id,
        },
      );
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col h-full min-h-[500px]">
          ...
          <table className="w-full border-collapse text-left relative">
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-50 border-b border-slate-100 shadow-sm relative z-20">
                ...
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider bg-slate-50 w-[300px] min-w-[300px]">
                  報警解決方法
                </th>
                ...
                paginatedAlarms.map((mapping) => (
                  ...
                  >
                    <td className="px-6 py-3 text-sm font-mono text-slate-600 align-top">
                      <div className="line-clamp-3" title={mapping.address}>
                        {mapping.address}
                      </div>
                    </td>
                    <td className="px-6 py-3 text-sm text-slate-600 align-top">
                      <div
                        ...
                        title={mapping.alarmBit.toString()}
                      >
                        {mapping.alarmBit}
                      </div>
                    </td>
                    <td className="px-6 py-3 text-sm font-medium text-slate-700 align-top">
                      <div className="line-clamp-3" title={mapping.code}>
                        {mapping.code}
                      </div>
                    </td>
                    <td className="px-6 py-3 text-sm text-slate-600 align-top">
                      <div className="line-clamp-3" title={mapping.content}>
                        {mapping.content}
                      </div>
                    </td>
                    <td className="px-6 py-3 text-sm text-slate-600 align-top">
                      <div className="line-clamp-3" title={mapping.level1Alarm}>
                        {mapping.level1Alarm}
                      </div>
                    </td>
                    <td className="px-6 py-3 text-sm text-slate-600 align-top">
                      <div className="line-clamp-3" title={mapping.level2Alarm}>
                        {mapping.level2Alarm}
                      </div>
                    </td>
                    <td className="px-6 py-3 text-sm text-slate-600 align-top w-[300px] min-w-[300px]">
                      <div className="line-clamp-3" title={mapping.solution}>
                        {mapping.solution}
                      </div>
                    </td>
      ...
      <div className="mt-4 flex items-center justify-between text-xs text-slate-400 px-2">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span>
            {alarmCurrentPage} / {Math.max(1, alarmTotalPages)}
          </span>
          <button
            onClick={() =>
              setAlarmCurrentPage((p) => Math.min(alarmTotalPages, p + 1))
            }
            disabled={
              alarmCurrentPage === alarmTotalPages || alarmTotalPages === 0
            }
            className="px-2 py-1 border rounded hover:bg-slate-50 disabled:opacity-50"
          >
            下一頁
        ...
        <p>
          支持 CSV 格式導入 (格式: 地址, 報警位, 代碼, 內容, 解決方法, 一級報警,
          二級報警)
        </p>
      </div>
    </div>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex space-x-3">
            {isProcessMappingEditing ? (
              <button
                onClick={saveProcessMappingEdit}
                className="flex items-center px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium transition-all shadow-sm"
              >
              ...
              <>
                ...
                >
                  <RotateCw
                    size={16}
                    className={`mr-2 ${isProcessRefreshing ? "animate-spin" : ""}`}
                  />
            ...
            <table className="w-full border-collapse text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {Array.from({ length: totalLength }).map((_, index) => {
                  ...
                  const displayMapping = isProcessMappingEditing
                    ? tempProcessMappings[index]
                    : baseMapping;
                  ...
                  return (
                    <tr
                      ...
                      draggable={
                        isProcessMappingEditing && !!displayMapping
                          ? true
                          : false
                      }
                      onDragStart={(e) => {
                        if (isProcessMappingEditing && displayMapping) {
                          handleDragStartProcess(index);
                        } else {
                          e.preventDefault();
                        }
                      }}
                      onDragOver={
                        isProcessMappingEditing
                          ? handleDragOverProcess
                          : undefined
                      }
                      onDrop={(e) => {
                        e.preventDefault();
                        if (isProcessMappingEditing) handleDropProcess(index);
                      }}
                    >
                      <td className="px-6 py-4 text-sm font-mono text-slate-600 bg-slate-50/50">
                        {autoAddress}
                      </td>
                      <td className="px-6 py-4 text-sm text-slate-500 bg-slate-50/50">
                        {autoBit}
                      </td>
                      <td
                        className={`px-6 py-4 text-sm font-medium text-slate-800 ${isProcessMappingEditing ? "pointer-events-none" : ""}`}
                      >
                        {displayMapping?.name || "-"}
                      </td>
                      <td
                        className={`px-6 py-4 text-sm text-slate-500 ${isProcessMappingEditing ? "pointer-events-none" : ""}`}
                      >
                        {displayMapping?.description || "-"}
                      </td>
                      <td
                        className={`px-6 py-4 text-sm text-slate-500 ${isProcessMappingEditing ? "pointer-events-none" : ""}`}
                      >
                        <span
                          className={`px-2 py-1 rounded text-xs ${displayMapping?.function === "Write" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
                        >
                      ...
                      <td
                        className={`px-6 py-4 text-sm text-slate-500 ${isProcessMappingEditing ? "pointer-events-none" : ""}`}
                      >
                        {displayMapping?.channelNumber || "-"}
                      </td>
                      <td
                        className={`px-6 py-4 text-sm font-medium text-indigo-600 ${isProcessMappingEditing ? "pointer-events-none" : ""}`}
                      >
                        {displayMapping?.parameterType || "-"}
                      </td>
                    </tr>
                  );
                })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, agvOrderRequestUrl: e.target.value })
              }
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, agvOrderEndUrl: e.target.value })
              }
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, agvOrderClearUrl: e.target.value })
              }
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700 flex items-center text-xs">
              <Activity size={14} className="mr-2 text-indigo-500" /> 修訂優先級
              URL
            </label>
            <input
              type="text"
              ...
              onChange={(e) =>
                setFormData({
                  ...formData,
                  agvOrderPriorityUrl: e.target.value,
                })
              }
                  ...
                  storageLocations.map((loc, idx) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-3 text-sm">
                        ...
                        >
                          <option value={StorageRequestType.Loading}>
                            上料
                          </option>
                          <option value={StorageRequestType.Unloading}>
                            下料
                          </option>
                        </select>
                      </td>
                      <td className="px-6 py-3 text-sm">
                        <select
                          value={loc.priority}
                          onChange={(e) =>
                            handleUpdateStorage(idx, "priority", e.target.value)
                          }
                          className={`bg-transparent border-b border-transparent hover:border-slate-200 focus:border-blue-500 outline-none py-1 font-medium ${
                            loc.priority === StoragePriority.VeryUrgent
                              ? "text-red-600"
                              : loc.priority === StoragePriority.Urgent
                                ? "text-amber-600"
                                : "text-slate-600"
                          }`}
                        >
                          ...
                          <option value={StoragePriority.VeryUrgent}>
                            非常緊急
                          </option>
                        </select>
                      </td>
              ...
              <>
                <div className="space-y-1">
                  <label className="text-sm font-semibold text-slate-700 flex items-center">
                    <Settings size={14} className="mr-1.5 text-blue-500" />{" "}
                    打卡機名稱
                  </label>
                  <input
                    type="text"
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="text"
                    value={formData.equipmentSN}
                    onChange={(e) =>
                      setFormData({ ...formData, equipmentSN: e.target.value })
                    }
                    ...
                    placeholder="例如: ASSET-2024-001"
                  />
                </div>
                <div className="md:col-span-2 space-y-1">
                  <label className="text-sm font-semibold text-slate-700 flex items-center">
                    <Fingerprint size={14} className="mr-1.5 text-blue-500" />{" "}
                    指紋儀編號
                  </label>
                  <select
                ...
                <div className="md:col-span-2 space-y-1">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all resize-none"
              ...
              <>
                ...
                <div className="md:col-span-2 space-y-1">
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none min-h-[80px]"
                  />
                </div>
              </>
            )}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-1">
                  <label className="text-sm font-semibold text-slate-700 flex items-center">
                    <Radio size={14} className="mr-1.5 text-indigo-500" /> PLC
                    品牌
                  </label>
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const newSeries =
                        newBrand === "Inovance" ? "H5U" : "MC-3E-ASCII";
                      setFormData({
                        ...formData,
                        plcBrand: newBrand,
                        plcSeries: newSeries,
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, plcSeries: e.target.value })
                    }
                    className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none bg-white transition-all"
                  >
                ...
                <div className="space-y-1">
                  <label className="text-sm font-semibold text-slate-700 flex items-center">
                    <Globe size={14} className="mr-1.5 text-indigo-500" /> IP
                    地址
                  </label>
                  <input
                    type="text"
                ...
                <div className="space-y-1">
                  <label className="text-sm font-semibold text-slate-700 flex items-center">
                    <Zap size={14} className="mr-1.5 text-indigo-500" />{" "}
                    通訊端口
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, plcPort: e.target.value })
                    }
                    className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-mono"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-semibold text-slate-700 flex items-center">
                    <Hash size={14} className="mr-1.5 text-indigo-500" /> 站號
                  </label>
                  <input
                    type="text"
                    value={formData.plcStation}
                    onChange={(e) =>
                      setFormData({ ...formData, plcStation: e.target.value })
                    }
                    className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-mono"
                    placeholder="PLC 站號"
                  />
                ...
                <div className="space-y-1">
                  <label className="text-sm font-semibold text-slate-700 flex items-center">
                    <Columns size={14} className="mr-1.5 text-indigo-500" />{" "}
                    數據類型
                  </label>
                  <select
                    value={formData.plcDataType}
                    onChange={(e) =>
                      setFormData({ ...formData, plcDataType: e.target.value })
                    }
                    className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none bg-white transition-all"
                  >
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            plcStringReverse: e.target.checked,
                          })
                        }
                      />
                      <div
                        className={`block w-10 h-6 rounded-full transition-colors ${formData.plcStringReverse ? "bg-indigo-600" : "bg-slate-300"}`}
                      ></div>
                      <div
                        className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${formData.plcStringReverse ? "translate-x-4" : ""}`}
                      ></div>
                    </div>
                    <span className="ml-3 text-sm font-semibold text-slate-700 group-hover:text-indigo-600 transition-colors">
                      字符串顛倒
                    </span>
                  </label>
                </div>
              ...
              {(device.type === EquipmentType.AssemblyEquipment ||
                ...
                <div className="mt-8 pt-6 border-t border-slate-100">
                  <h4 className="text-sm font-bold text-slate-800 flex items-center mb-6">
                    <Database size={16} className="mr-2 text-indigo-600" /> PLC
                    數據地址配置
                  </h4>
                        ...
                        <div className="space-y-1">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                alarmReadMethod: e.target.value,
                              })
                            }
                        ...
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                alarmEndAddress: e.target.value,
                              })
                            }
                            className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-mono text-sm"
                        ...
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                processParamAddress: e.target.value,
                              })
                            }
                            className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none font-mono text-sm"
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <div className="flex items-center text-sm text-slate-500 mt-1">
              <span className="font-mono bg-slate-100 px-2 py-0.5 rounded text-slate-600 mr-2">
                ID: {device.id}
              </span>
              <span className="flex items-center">
                <Cpu size={14} className="mr-1" /> {device.type}
              </span>
            </div>
          </div>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium shadow-sm transition-all active:scale-95 disabled:opacity-50"
        >
          {saving ? (
            <span className="flex items-center">
              <RotateCw className="animate-spin mr-2" size={18} /> 處理中...
            </span>
          ) : (
            <span className="flex items-center">
              <Save className="mr-2" size={18} /> 保存配置
            </span>
          )}
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 px-2">
        <div className="flex overflow-x-auto no-scrollbar">
          <button
            ...
            className={`px-6 py-4 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === "BASIC" ? "border-blue-600 text-blue-600" : "border-transparent text-slate-500 hover:text-slate-700"}`}
          >
            基礎信息與通訊
          </button>
          <button
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 min-h-[500px]">
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                ...
                {processMappings.filter((m) => m.parameterType === "工藝排產")
                  .length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {processMappings
                      .filter((m) => m.parameterType === "工藝排產")
                      .map((mapping, idx) => (
                        <button
                          key={mapping.id}
                        ...
                        >
                          <div className="flex items-start">
                            <div
                              className={`w-8 h-8 rounded-full border flex items-center justify-center font-bold text-xs mr-3 shrink-0 ${
                                selectedScheduleItem === mapping.id
                            ...
                            <div>
                              <p
                                className={`text-sm font-medium ${selectedScheduleItem === mapping.id ? "text-indigo-900" : "text-slate-800"}`}
                              >
                                排產任務節點
                              </p>
                              <p
                                className={`text-xs font-mono mt-1 break-all ${selectedScheduleItem === mapping.id ? "text-indigo-700" : "text-slate-500"}`}
                              >
                                地址: {mapping.address} | 位:{" "}
                                {mapping.parameterBit}
                              </p>
                            </div>
                          </div>
                        </button>
                  ...
                  scheduleFactors.length > 0 ? (
                    <div className="space-y-4">
                      {scheduleFactors.map((factor) => (
                        <div
                          key={factor.equipmentId}
                          className="bg-slate-50 rounded-xl p-5 border border-slate-200"
                        >
                          <h5 className="font-semibold text-slate-700 mb-4">
                            {factor.equipmentName}
                          </h5>
                          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            {factor.results.map((res) => (
                              <label
                                key={res.id}
                                className="flex items-center space-x-3 bg-white p-3 rounded-lg border border-slate-200 cursor-pointer hover:border-indigo-300 transition-colors"
                              >
                                <input
                                  type="checkbox"
                                  checked={res.value}
                                  onChange={() => {
                                    ...
                                    const fIdx = updated.findIndex(
                                      (f) =>
                                        f.equipmentId === factor.equipmentId,
                                    );
                                    if (fIdx > -1) {
                                      const rIdx = updated[
                                        fIdx
                                      ...
                                      if (rIdx > -1) {
                                        updated[fIdx].results[rIdx].value =
                                          !updated[fIdx].results[rIdx].value;
                    ...
                    <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50">
                      <Layout
                        size={32}
                        className="mx-auto text-slate-300 mb-3 opacity-50"
                      />
                      <p className="text-sm font-medium text-slate-600">
                        無前置設備
                      </p>
                ...
                <div className="border border-slate-200 rounded-lg overflow-hidden max-h-60 overflow-y-auto bg-slate-50 custom-scrollbar">
                  <div className="grid grid-cols-1 divide-y divide-slate-100">
                    {SYSTEM_PROCESS_PARAMETERS.map((param) => (
                      ...
                      >
                        <div
                          className={`w-4 h-4 rounded-full border mr-3 flex items-center justify-center shrink-0 ${
                            selectedProcessType === param
                              ? "border-indigo-600"
                              : "border-slate-300"
                          }`}
                  ...
                  {newProcessDataType === "字符串" && (
                    <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-200">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewProcessDataLength(parseInt(e.target.value) || 0)
                        }
